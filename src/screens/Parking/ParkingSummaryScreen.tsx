...
export const ParkingSummaryScreen: React.FC = () => {
  const theme = useTheme();
  const navigation = useNavigation();
  const route = useRoute();
  const { session, reminderEnabled = false, reminderOffsetMinutes = 10 } = (route.params || {}) as RouteParams;
  ...
  return (
    <>
      <AppHeader title={t("parking.summary")} variant="stack" />
          ...
          {priceLoading ? (
            <Card style={{ marginBottom: theme.spacing.s16 }}>
              <View style={{ padding: theme.spacing.s16, alignItems: "center" }}>
                ...
                <Text style={[textStyles.caption, { color: theme.colors.textSecondary, marginTop: theme.spacing.s8 }]}>
                  {t("common.loading")}
                </Text>
              </View>
            </Card>
